...
let selectedTime = null;
    ...
    function updateAppointmentTime() {
        if (selectedDate && selectedTime) {
            // Combine thành định dạng ISO 8601: "2025-08-04T14:30"
            const combined = `${selectedDate}T${selectedTime}`;
            document.getElementById('appointment_time').value = combined;
        console.log("Giá trị sẽ gửi lên appointment_time:", combined);
        }
    }

    // Ví dụ: khi người dùng bấm chọn ngày
    document.querySelectorAll('.date-container button').forEach(btn => {
        btn.addEventListener('click', function () {
            selectedDate = this.dataset.date;  // ví dụ: "2025-08-04"
    ...
    document.querySelectorAll('.time-container button').forEach(btn => {
        btn.addEventListener('click', function () {
            selectedTime = this.dataset.time;  // ví dụ: "14:30"
            updateAppointmentTime();
        });
});